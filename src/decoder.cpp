...
Packet Decoder::decodeContextPacket() {
  ...
  const bool has_payload =
      (this->trace_data[this->trace_data_offset] & 0x1) ? true : false;
  ...
  const bool has_virtual_context =
      (this->trace_data[this->trace_data_offset + 1] & 0b01000000) ? true
                                                                   : false;
...
Packet Decoder::decodeExceptionPacket() {
  ...
  // Otherwise, if c is not set, there are no more exception information bytes
  // in the packet.
