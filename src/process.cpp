    ...
    const std::uint8_t* trace_data_addr, const std::size_t trace_data_size)
{
    ...
    while (this->decoder.trace_data_offset < size) {
        ...
        // The length of the packet data is insufficient and decoding cannot be performed correctly at this time.
        // In this case, the decoding process is put to rest and new data is received.
        ...
        switch (this->decoder.state) {
            ...
            case DecodeState::TRACE: {
                switch (packet.type) {
                    ...
                    case ETM4_PKT_I_ATOM_F5:
                    case ETM4_PKT_I_ATOM_F6: {
                        // When processing an atom packet, there is an unprocessed indirect branch instruction.
                        // If there is an unprocessed indirect branch instruction, there must be an address packet, not an atom packet.
                        // When this error occurs, there is probably a bug in this program itself.
                        ...
                        assert(this->state.prev_location.has_value() == true);
                            ...
                            // Check for edge coverage in the cache, calculated from the same trace data and starting address.
                            // If it exists, we can skip the decoding process of a atom packet.
                    ...
                    case ETM4_PKT_I_ADDR_CTXT_L_64IS0: {
                        ...
                        //   1. Generated to indicate the trace start address at the start of the trace.
                        //   2. Generated following an atom packet (E) at an indirect branch instructions.
                        ...
                        //   b. Indicates the branch destination of an immediately preceding indirect branch instruction.
                        //
                        // In the case of b, there is no need to decode it, so ignore it.

                        if (state.has_pending_address_packet) {
                            // Check if the destination address jumped by the indirect branch is on the memory map.
                            const std::optional<AddressTrace> optional_trace =
                                processAddressPacket(packet);
                    ...
                    // An exception packet is generated when an exception occurs. Following the exception packet,
                    // two address packets are generated. The first shows the address to return after the exception,
                    // and the second shows the address where execution actually resumed after the exception.
                    ...
                    // A trace on packet indicates a discontinuity in the trace stream. After the trace on packet is generated,
                    // the trace unit generates an address packet to indicate the start of the trace before generating the next
...
AtomTrace Process::processAtomPacket(const Packet &atom_packet)
{
    ...
    for (std::size_t i = 0; i < atom_packet.en_bits_len; ++i) {
        ...
        // In the case of an indirect branch instruction, an atom packet (E) and an address packet are generated.
        // Therefore, after consuming the atom packet, the address packet is processed.
        if (insn.type == INDIRECT_BRANCH) {
            ...
            // Next, it is expected that an address packet, which indicates the jump destination address of the indirect branch, will be processed.
...
BranchInsn Process::processNextBranchInsn(const Location &base_location)
{
    ...
    BranchInsn insn; {
            ...
            // Access the cache and check if the same offset of the same memory image has already been disassembled.
            //  If the data exists in the cache, there is no need to disassemble it.
            if (this->data.cache.isCachedBranchInsn(insn_key)) {
                // Since the results of the disassembly are already in the cache, read the data from the cache.
