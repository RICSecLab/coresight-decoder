...
void usage(const char* argv0) {
    std::cerr << "Usage: " << argv0 << " "
              ...
              << "[IMAGE_FILE_NUM] [IMAGE_FILE1] [IMAGE_FILE2] ...  [OPTIONS]" << std::endl
              << "OPTIONS:" << std::endl
              << "\t--output-filename=name : Specify a file to save the execution time results." << std::endl
              << "\t                         If it is not specified, no output is generated." << std::endl
              << "\t--loop-cnt=num         : Specify the number of times to perform repeated decoding " << std::endl
              << "\t                         for each trace data. The default value is 1." << std::endl
              << std::endl;
}
...
int main(int argc, char const *argv[])
{
    ...
    struct libcsdec_memory_image* libcsdec_memory_image =
        (struct libcsdec_memory_image*)malloc(sizeof(struct libcsdec_memory_image) * memory_image_num); {
        for (int i = 0; i < memory_image_num; ++i) {
            load_bin(argv[3 + trace_data_num + i], &libcsdec_memory_image[i].data, &libcsdec_memory_image[i].size);
        }
    }
    ...
    libcsdec_t libcsdec; {
        if (cov == Cov::Edge) {
            libcsdec = libcsdec_init_edge(local_bitmap, bitmap_size,
                memory_image_num, libcsdec_memory_image);
        } else if (cov == Cov::Path) {
    ...
    for (int time = 0; time < loop_cnt; ++time) {
        for (std::size_t i = 0; i < trace_data_dir.size(); i++) {
            const std::string decoder_args_path = trace_data_dir[i] + "/decoderargs.txt";
